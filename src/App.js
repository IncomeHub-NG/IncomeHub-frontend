    ...
    <div style={{minHeight:"100vh",background:"#0F1419",fontFamily:"'Segoe UI',-apple-system,sans-serif",color:"#FFFFFF",overflowY:"auto"}}>
      <div style={{width:"100%",maxWidth:480,margin:"0 auto",paddingBottom:90}}>
        ...
        <div style={{background:"linear-gradient(135deg,#0d2b1a,#1a3d2b)",padding:"32px 16px 20px"}}>
          ...
          {/* Points Balance Card */}
          <div style={{background:"rgba(0,0,0,.3)",borderRadius:14,padding:16,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            <div style={{textAlign:"center"}}>
              <p style={{margin:"0 0 2px",fontSize:22,fontWeight:900,color:"#2ECC71"}}>{totalPoints.toLocaleString()}</p>
              <p style={{margin:0,fontSize:10,color:"rgba(255,255,255,.5)"}}>My Points</p>
            </div>
            <div style={{textAlign:"center",borderLeft:"1px solid rgba(255,255,255,.1)",borderRight:"1px solid rgba(255,255,255,.1)"}}>
              <p style={{margin:"0 0 2px",fontSize:22,fontWeight:900,color:"#F39C12"}}>₦{pointsToNaira(totalPoints).toLocaleString()}</p>
              <p style={{margin:0,fontSize:10,color:"rgba(255,255,255,.5)"}}>Cash Value</p>
            </div>
            <div style={{textAlign:"center"}}>
              <p style={{margin:"0 0 2px",fontSize:22,fontWeight:900,color:"#85C1E9"}}>{claimedTasks.length}</p>
              <p style={{margin:0,fontSize:10,color:"rgba(255,255,255,.5)"}}>Completed</p>
            </div>
          </div>

          {/* Points info */}
          <div style={{marginTop:10,padding:"8px 12px",background:"rgba(46,204,113,.08)",borderRadius:8,fontSize:11,color:"rgba(255,255,255,.6)",textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:8,padding:"16px 16px 8px",overflowX:"auto",scrollbarWidth:"none"}}>
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
              padding:"10px 18px",
              borderRadius:20,
              border: activeTab===tab.key ? "none" : "1px solid rgba(255,255,255,.12)",
              background: activeTab===tab.key ? "#2ECC71" : "transparent",
              color: activeTab===tab.key ? "#0F1419" : "#FFFFFF",
              fontSize:12,
        ...
        <div style={{padding:"4px 16px 12px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:11,color:"#A0AEC0"}}>{currentTasks.filter(t=>isCompleted(t.id)).length} of {currentTasks.filter(t=>t.points).length} tasks completed</span>
            ...
            return (
              <div key={task.id} style={{
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                  ...
                  <div style={{
                    width:44,height:44,
                    background: done ? "rgba(46,204,113,.2)" : "rgba(255,255,255,.05)",
                    borderRadius:12,
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
                      <p style={{margin:0,fontSize:13,fontWeight:700,color:done?"#2ECC71":"#FFFFFF",flex:1,paddingRight:8}}>{task.title}</p>
                      {task.points && (
                        <div style={{background: done ? "rgba(46,204,113,.15)" : "rgba(243,156,18,.15)",borderRadius:20,padding:"3px 8px",whiteSpace:"nowrap"}}>
                          <span style={{fontSize:11,fontWeight:700,color:done?"#2ECC71":"#F39C12"}}>+{task.points} pts</span>
                        </div>
                      )}
                    </div>
                    <p style={{margin:"0 0 10px",fontSize:11,color:"#A0AEC0",lineHeight:1.5}}>{task.desc}</p>

                    {/* Action Buttons */}
                    {done ? (
                      <div style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:11,color:"#2ECC71",fontWeight:600}}>
                        ✓ Completed — points added to your balance
                      </div>
                    ) : verif ? (
                      <div style={{display:"flex",gap:8}}>
                        <button onClick={() => claimTask(task)} style={{
                          flex:1,background:"#2ECC71",color:"#0F1419",border:"none",
                        ...
                        <button onClick={() => setVerifying(null)} style={{
                          background:"rgba(255,255,255,.08)",color:"#A0AEC0",border:"none",
                      ...
                      <button onClick={() => handleTask(task)} style={{
                        background: task.action==="offerwall" && !task.comingSoon
                          ...
                          : "rgba(46,204,113,.12)",
                        color: task.action==="offerwall" ? "#FFFFFF" : "#2ECC71",
        ...
        {totalPoints >= 5000 ? (
          <div style={{margin:"8px 16px 20px",background:"linear-gradient(135deg,#2ECC71,#1a9e53)",borderRadius:14,padding:16,textAlign:"center"}}>
            <p style={{margin:"0 0 4px",fontSize:14,fontWeight:800,color:"#0F1419"}}>🎉 You can withdraw!</p>
            ...
            <button onClick={() => alert("Points withdrawal coming soon! We will notify you when it goes live.")} style={{background:"#0F1419",color:"#2ECC71",border:"none",borderRadius:20,padding:"10px 24px",fontSize:13,fontWeight:700,cursor:"pointer"}}>
              Withdraw Points
            </button>
          </div>
        ) : (
          <div style={{margin:"8px 16px 20px",background:"rgba(46,204,113,.05)",border:"1px solid rgba(46,204,113,.15)",borderRadius:14,padding:14,textAlign:"center"}}>
            <p style={{margin:"0 0 4px",fontSize:12,color:"#A0AEC0"}}>
              You need <span style={{color:"#2ECC71",fontWeight:700}}>{(5000 - totalPoints).toLocaleString()} more points</span> to withdraw
            </p>
            <div style={{height:6,background:"rgba(255,255,255,.06)",borderRadius:3,margin:"8px 0 0"}}>
              <div style={{height:6,background:"#2ECC71",borderRadius:3,width:Math.min(100,(totalPoints/5000)*100)+"%",transition:"width .5s"}} />
            </div>
          </div>
        )}
